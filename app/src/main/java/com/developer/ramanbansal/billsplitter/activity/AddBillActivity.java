...
import com.developer.ramanbansal.billsplitter.adapter.MemberAdapter;
...
public class AddBillActivity extends AppCompatActivity implements View.OnClickListener, AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Cursor cursor = memberAdapter.getMember(expenId);
        cursor.moveToFirst();
        while (!cursor.isAfterLast()) {
            listMember.add(new Member(cursor.getString(cursor.getColumnIndexOrThrow(Helper.COLUMN_MEMBER_NAME)), Integer.parseInt(expenId)));
            cursor.moveToNext();
        }

        Cursor result = memberAdapter.getMember(expenId);
    ...
    private void insertMember() {
        ...
        for (int i = 0; i < noPerson; i++) {
            double temp = memberAdapter.getBalance(expenId, listMember.get(i).getName());
            amount[i] = temp + listMember.get(i).getCredit() - listMember.get(i).getDebit();
            listMember.get(i).setBalance(amount[i]);
            memberAdapter.updateBalance(expenId, listMember.get(i).getName(), listMember.get(i).getBalance());
    ...
    private void insertBill(double share, String members) {
        ...
        billAdapter.createBill(etName.getText().toString(), etAmount.getText().toString(), share, members, expenId);
